    ...
    public static class ATS_TMPTools
    {
            ...
            int iSpriteSize = 64, int iMaxAtlasSize = 4096)
        {
            ...
            for (int i = 0; i < iTextures.Count; i++)
            {
                ...
                var cols = UCL.Core.TextureLib.Lib.GetPixels(iTextures[i], iSpriteSize, iSpriteSize);
                var new_texture = new Texture2D(iSpriteSize, iSpriteSize, iTextures[i].format, true);
                new_texture.SetPixels(cols);
                aTextures[i] = new_texture;
            }
